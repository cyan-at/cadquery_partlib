...
edge_x = x_center + servo_block_dx / 2
shaft_center_x = edge_x - 11
# negate above to switch 'sides'

shaft_center_y = y_center
shaft_diameter = M5
result = result.faces(">Z").workplane()\
    .moveTo(shaft_center_x, shaft_center_y)\
    .circle(shaft_diameter / 2)\
    .cutBlind(-thickness)

# save on filament
result = result.faces(">Z").workplane()\
    .moveTo(x_center - 10, y_center)\
    .rect(20, servo_block_dy)\
    .cutBlind(-thickness)
...
cq.exporters.export(result,"./%s.stl" % ("servo_mount_right"))
