...
class GeoUtil:
  ...
  def ch_gift_wrapping_jarvis_march(ps):
    ...
    cw_hull_pts = []
    hull_pt_indices = []
    cw_hull_pts.append(left_most_point)
...
def source_sink_point(
  source_point2, source_height,
  sink_point2, sink_height,
  target_height):
  s = np.array([source_point2.x, source_point2.y])
  e = np.array([sink_point2.x, sink_point2.y])
  vec = e - s

  total_dt = sink_height - source_height
  dt = target_height - source_height
  scale = dt / total_dt

  return s + vec * scale

def find_alive_contour_points(skeleton, target_height):
  '''
    skeleton: list[Subtrees of source, sinks, height]
    target_height:
      >= max_height (no contour)
      < max_height >= 0 (inside polygon)
      < 0 (outside polygon)
  '''
  contour_points = []
  ...
  point_to_height = {}
  for arc in skeleton:
    point_to_height[arc.source] = arc.height

  for arc in skeleton:
    parent_height = point_to_height[arc.source]

    for sink in arc.sinks:
      sink_height = 0.0
      if sink in point_to_height:
        sink_height = point_to_height[sink]

      if target_height >= sink_height and target_height <= parent_height:
        # this is an 'alive edge' INSIDE the polygon
        contour_points.append(
          source_sink_point(
            arc.source, parent_height,
            sink, sink_height,
            target_height)
        )
      elif sink_height == 0.0 and target_height <= 0.0:
        # sink_height == 0.0 => leaf node
        # target_height <= 0.0 => outside the polygon
        contour_points.append(
          source_sink_point(
            arc.source, parent_height,
            sink, sink_height,
...
def polygon_offset(cw_hull_pts, factor):
  '''
  factor:
  >= 1 (no contour)
  < 1 >= 0 (inside polygon)
  ...
  skeleton = skeletonize(cw_hull_pts, [])
  max_height = max([s.height for s in skeleton])
  contour_points = find_alive_contour_points(skeleton, factor * max_height)

  # unfortunately, there is no guarantee in the ordering of the skeleton
  # so you need to hull to sort by polarity
  contour_points, _, _ = GeoUtil.ch_gift_wrapping_jarvis_march(contour_points)
